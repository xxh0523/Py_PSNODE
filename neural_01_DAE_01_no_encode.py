...
class DAE_Model(nn.Module):
    ...
    def forward(self, t: torch.Tensor, x: torch.Tensor, z: torch.Tensor, v: torch.Tensor, i: torch.Tensor, event_t: torch.Tensor, z_jump: torch.Tensor, v_jump: torch.Tensor):
        ...
        all_initial = torch.cat((x.permute(1,0,2)[0], z.permute(1,0,2)[0], v.permute(1,0,2)[0], i.permute(1,0,2)[0]), dim=-1)
        # batch, time, variable -> time, batch, variable
        x_solution, i_solution = self.solver.integrate_DAE(x_func=self.de_func,
                                                           i_func=self.ae_func,
                                                           t=t.permute(1,0,2), 
                                                           x=x.permute(1,0,2), 
                                                           z=z.permute(1,0,2), 
