...
from neural_dae.neural_base import AE_Func
import torch
...
is_training = False
is_testing = False
p_model = '/home/xiaotannan/pythonPS/00saved_results/models/neural_dae/neural_avr_1_20211210_6_2/model_checkpoint.50'
    ...
    if args.training:
        ...
        for epoch in tqdm(range(1, num_epoch+1), desc='Epoch', ncols=ncols):
            ...
            for i_batch, data_batch in enumerate(tqdm(training_dataloader, desc=f'Epoch {epoch} Training', leave=False, ncols=ncols)):
                ...
                # cal loss
                loss = torch.sum(Loss_func(x_pred, x, reduction='none') * mask) / torch.sum(mask)
